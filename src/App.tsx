...
export default function App() {
  ...
  const login = async (next: Credentials) => {
    ...
    try {
      ...
      if (needsSetup) {
        ...
        setNotice('Настройки уведомлений обновлены. GREEN-API применяет их до 5 минут — входящие могут появиться с задержкой.')
      }
      saveCredentials(next)
      setCreds(next)
    } catch (e) {
      setLoginError(e instanceof GreenApiError && e.status === 400 ? 'Проверьте idInstance: ожидается число' : describe(e))
    } finally {
      setLoginBusy(false)
    }
  }
  ...
  const send = async (text: string) => {
    ...
    dispatch({ type: 'sendStart', key: chat.key, tempId, text, timestamp: Date.now() })
    try {
      const res = await client.sendMessage(chat.chatId, text)
      if (!res?.idMessage) throw new Error('GREEN-API не вернул idMessage')
      dispatch({ type: 'sendDone', key: chat.key, tempId, idMessage: res.idMessage })
    } catch (e) {
